...
- `--ui-backend <name>` / `--ui-keys <string>` / `--ui-rows N` / `--ui-cols N` / `--ui-snapshot <path>`: headless plumbing for `--ui curses`. `--ui-keys` converts the string char-by-char into a byte queue passed as `opts.keys` to the curses driver's buffer backend. `--ui-snapshot` writes the final `backend:snapshot()` to a file after `engine.run` returns; silently skipped when the active driver has no `_backend_handle`. All five flags are pass-through fields on `driver_opts`, so drivers that do not recognise them (plain, ansi, curses-with-ncurses) ignore them.
...
Plain-text UI driver (default). Renders narration as `"Speaker: text"` or bare text; numbered choice prompt; reads from stdin. §M5/§M8: `attach(kernel)` discovers author-declared widget bindings (state `ui:` blocks + `ui-panel` children) and prefixes every `render` with one text line per binding, re-read from the kernel state cache (no mutation subscription — the driver is synchronous-pull). Flattens `stat-bar` / `stat` to `"Label: value"` via `ui/format.value`, `toggle` to `"Label: yes|no"`, `inventory` to `"Label: a, b, c"` (or `"Label: (empty)"`), and `text` panel children to their interpolated string (`{path}` placeholders substituted through the same formatter). Unknown widget kinds are skipped. `detach` drops the bindings.
...
Curses UI driver (`--ui curses`) — pull-mode for narration + choices, reactive for §M5/§M8 widgets, with a §M6 view-stack for overlays. Composes one frame per `render`/`prompt`: a variable-height widget strip (top, stacked top-to-bottom by `widget._desired_h`) / narration tail / separator / numbered choices / prompt status line, then each active view (e.g. modal dialog) painted on top via `_view_stack:each_for_paint`, into a retained 2D `screen.lua` grid; `paint.lua` diff-flushes only changed cells. The widget strip is capped so narration always gets ≥1 row; per-widget heights default to 1 (stat-bar / stat / toggle / text) and grow with item count for inventory / choice. `opts.backend` selects "ncurses" (default) or "buffer" (headless tests, no-tty fallback). `opts.frame_history` opts the buffer backend into per-flush snapshot + dirty-rect recording. `M.new` is lazy: `backend:init` is deferred to the first `render`/`prompt` so `is_driver()` checks work without a TTY. `opts.curses` injects a stub curses module for the ncurses backend; `opts.keys` pre-loads the buffer backend's key queue. `attach(kernel)` scans the schema (state `ui:` blocks) and `ui_panels` (children with state-path refs or text templates) and instantiates a widget per binding via a `ui.kind → constructor` dispatch table covering `stat-bar`, `stat`, `toggle`, `text`, `inventory`, `choice`; unknown kinds are silently skipped. Subscribes to `mutation` on every bound path (text widgets register under every placeholder path) so a state change recomposes the frame — windows outside the affected widget re-paint identically and diff clean, so the dirty-rect log covers only the widgets that actually changed. §M6 input dispatch in `_read_choice`: keys route to the top focus handler first (modal dialogs intercept); if not consumed, q/Q/ESC open a quit-confirm dialog via `_open_quit_dialog` (Yes sets `_quit_pending` and returns nil from prompt; No/ESC just pop the modal); digits 1..#choices resolve the prompt. `detach` clears view + focus stacks, drops subscriptions, and tears down the backend. Registered in the `cli.ui` enum (`runtime/engine.lua`).

### `cli/drivers/curses/screen.lua`
Retained 2D cell grid + window model shared by both backends (ui_idea.md §8.1). Cells are `{ ch, fg?, bg?, attrs? }`. `screen.new(rows, cols)` allocates the grid; `add_window({x,y,w,h,z?,title?})` allocates a window; `write(win, wy, wx, s, attrs?)` paints a string (clipped to the window and the screen); `fill(win, wy, wx, w, h, ch, attrs?)` blankets a rectangle; `clear()` resets the current frame; `commit()` promotes the frame to `_prev` after a backend flush; `invalidate()` forces full repaint on the next diff; `as_lines()` returns the frame as row strings.

### `cli/drivers/curses/paint.lua`
Cell-grid diff. `M.diff(screen)` walks every row of `screen._prev` vs `screen._cells` and returns the minimal list of `{y, x, cells}` dirty-rect runs (contiguous spans of changed cells). `M.rect_to_string(cells)` joins a rect's glyphs into a string the backend can paint in one call. Returns an empty list when nothing changed (the common idle-frame case).

### `cli/drivers/curses/backend_curses.lua`
ncurses backend for the curses driver — **the only file in the project that `require("curses")`**. M3 interface: `init(rows?, cols?)` calls `initscr`/`cbreak`/`echo(false)`/`curs_set(0)`/`keypad(true)` (idempotent); `size()` returns the terminal geometry via `getmaxyx`; `flush(screen, dirty_rects)` emits one `mvaddstr` per dirty rect and calls `refresh()` (refresh fires even when the dirty list is empty to keep the cursor positioned); `read_key()` is one blocking `getch`; `close()` calls `endwin` and is idempotent.

### `cli/drivers/curses/backend_buffer.lua`
String-buffer backend with the same M3 interface (`init(rows, cols)`/`size`/`flush`/`read_key`/`close`) plus test-side extras: `snapshot()` joins the buffer into one newline-separated string with per-row trailing-space trimming; `snapshot_rows()` returns raw row strings; `script_keys({…})` pre-loads the `read_key` queue (strings are converted character-by-character to byte codes, integers pass through). §M5 frame-history (opt-in `frame_history = true` on `M.new`): each `flush()` pushes a `snapshot()` row-string + a deep copy of the dirty rect list onto `_hist_snapshots` / `_hist_dirty`, surfaced via `snapshot_history()` / `dirty_history()` / `reset_history()` for before/after-mutation assertions in widget specs. Used by `tests/ui/curses_screen_spec.lua` for golden snapshots and as a no-tty fallback when `lcurses` is missing.

### `cli/drivers/curses/widgets/statbar.lua`
§M5 stat-bar widget. `M.new({path, label?, type_desc, color_good?, color_bad?, min_bar_w?})` returns a widget bound to a single state path. `widget:paint(screen, win, kernel)` reads `kernel:get_state(path)`, erases the window, writes `"Label: cur/max "` (via `ui/format.lua:int`; nil values render as `"Label: -"`), then a bracketed progress bar — `#` cells proportional to value/max (rounded; clamped to [0, max]) and ` ` for the empty portion. Optional `color_good` / `color_bad` are passed as the `fg` cell attribute. Narrow windows that can't fit the bar render the head fragment truncated; zero-height windows are a no-op. Driver-agnostic API: the screen + window abstraction is the same one the M3 screen model uses, so the widget could be reused by any future curses-like driver.

### `cli/drivers/curses/widgets/dialog.lua`
§M6 modal dialog widget. `M.new({title?, body, buttons, on_select?, on_cancel?, min_w?, max_w?, z?})` returns an object that acts as both a *view* (paints into a centred bordered box sized to its contents — ASCII box-drawing) and a *focus handler* (intercepts keys). Buttons: `{key=<char|byte>, label, action?}`; matched key fires `action(driver)` then dialog-wide `on_select(self, btn, driver)`. ESC / q / Q always consume and fire `on_cancel(self, driver)`. Key matching is case-insensitive. Layout auto-sizes inner width to max(title+4, body lines, button row), centres on the screen, and clips multi-line bodies on the right; zero-button dialogs render body-only.

### `cli/drivers/curses/view_stack.lua`
§M6 driver-local view stack (ui_idea.md §7). `view_stack.new()` returns a stack with `push(view, driver)` / `pop(driver)` / `top` / `depth` / `empty` / `clear(driver)` / `each_for_paint(fn)`. A view is a table with `paint(self, screen, ctx)` (required) and optional `on_open(self, driver)` / `on_close(self, driver)` lifecycle hooks fired on push/pop. **Distinct from the engine's scene stack**: opening an inventory popup or a quit-confirm modal does not change `eng._scene_stack`, isn't persisted in saves, and never fires `scene-change`. `each_for_paint` iterates bottom-to-top so the painter overlays modals correctly via the shared cell grid (later writes overwrite earlier cells).

### `cli/drivers/curses/focus.lua`
§M6 focus / key dispatch stack (ui_idea.md §8.3). Separate from view_stack because the two concerns can move independently (a HUD overlay paints without taking focus). `focus.new()` returns a stack with `push(handler, driver)` / `pop(driver)` / `top` / `depth` / `empty` / `clear(driver)` / `dispatch(key, driver)`. A handler is a table with `on_key(self, key, driver) -> handled` (required) and optional `on_focus` / `on_blur`. Dispatch routes only to the top handler — no fallthrough to handlers underneath — and normalises truthy non-boolean returns to `true`. Push/pop fire `on_blur`/`on_focus` on the affected handlers so widgets can flip cursor state.

### `cli/drivers/curses/keys.lua`
§M7 shared integer key-code constants for the curses driver's widgets and tests. ASCII codes (`ESC=27`, `CR=13`, `LF=10`, `BS=8`, `DEL=127`, `TAB=9`, `SPACE=32`) plus ncurses `KEY_*` codes (UP=259, DOWN=258, LEFT=260, RIGHT=261, BACKSPACE=263, F1..F4, HOME, END, DC, IC, NPAGE, PPAGE, ENTER=343) hard-coded against the ncurses headers — lcurses does not expose these as table fields, only as `getch()` return values. Convenience predicates: `is_enter(key)` (CR/LF/KEY_ENTER), `is_backspace(key)` (BS/DEL/KEY_BACKSPACE), `is_printable(key)` (ASCII 32..126).

### `cli/drivers/curses/widgets/menu.lua`
§M7 navigable menu widget. `M.new({title?, items, on_activate?, on_cancel?, border?, wrap?, min_w?, max_w?, z?, highlight_attrs?, header_attrs?, divider_glyph?, keys?})` returns an object acting as both a *view* (centred bordered box) and a *focus handler*. Items are `{kind?="item", label, hotkey?, action?}` / `{kind="divider"}` / `{kind="header", label}`. Cursor starts on the first selectable item and skips dividers/headers; Up/Down (defaults `KEY_UP`/`KEY_DOWN`) navigate with optional wrap (default on); Home/End jump to first/last selectable; Enter (CR/LF/KEY_ENTER) activates current and fires `item.action(driver, item)` then `on_activate(self, item, driver)`. Per-item hotkeys (single char) jump-and-activate. ESC fires `on_cancel`. Customisation: `highlight_attrs` (default `{attrs="reverse"}`), `header_attrs` (default `{attrs="bold"}`), `divider_glyph` (default `-`), per-action `keys` overrides where each binding accepts either an int or an array of ints (e.g. `keys={up={K.KEY_UP, string.byte("k")}}` to add vim navigation). Hotkeys are rendered inline as `[k] Label`. Border defaults to true; `border=false` paints items only. Submenu support is the author's job — an `action` can push another menu onto the driver's stacks.

### `cli/drivers/curses/widgets/input.lua`
§M7 single-line free-text input widget. `M.new({title?, prompt?, initial?, max_len?, fn_name?, on_submit?, on_cancel?, border?, min_w?, max_w?, z?, cursor_glyph?, keys?})` returns a view+focus-handler. Editing model: `_buffer` is the byte string, `_cursor` is a 1-based insertion point; printable bytes (32..126) insert, backspace/KEY_BACKSPACE/DEL delete previous, KEY_DC deletes under cursor, left/right move cursor, Home/End jump to ends, Enter (CR/LF/KEY_ENTER) submits, ESC cancels. On submit: when `fn_name` is set and the driver carries a `_kernel`, the widget calls `driver._kernel:call_fn(fn_name, text)` *before* invoking `on_submit(self, text, driver)`; this is the §6 input-vocabulary acceptance path. `max_len` caps the buffer (0 = unlimited; cap enforced by both `insert_byte` and `set_text`). The cursor glyph (`_` by default) renders in-band with reverse-video attrs since the buffer backend has no hardware cursor. Long buffers scroll to keep the cursor in view. `keys` overrides accept ints or int arrays.

### `cli/drivers/curses/widgets/stat.lua`
§M8 scalar-display widget. `M.new({path, label?, type_desc?, schema?, bool_labels?})` returns a widget bound to a single state path. `widget:paint(screen, win, kernel)` reads `kernel:get_state(path)`, erases the window, writes `"Label: <value>"` via `ui/format.value` — bounded ints render as "cur/max", bools as "yes/no", enums by symbol, named/alias chains resolved against the schema, nil values as `"Label: -"`. Long values are clipped to the window width; zero-height windows are no-ops. Use as the general read-only state display when `stat-bar` is too heavy.

### `cli/drivers/curses/widgets/toggle.lua`
§M8 Bool toggle widget. `M.new({path, label?, fn_name?, on_toggle?, on_glyph?, off_glyph?})` returns a view+focus-handler. Paint renders `"[x] Label"` or `"[ ] Label"` based on `kernel:get_state(path)` (nil → off); glyphs are single-char (defaults `x` / space). When pushed onto the focus stack, SPACE / Enter (CR / LF / KEY_ENTER) flip the value: if `fn_name` is set and the driver carries a `_kernel`, `kernel:call_fn(fn_name, new_value)` fires first; then the optional `on_toggle(self, new_value, driver)` callback. `activate(driver)` returns the new boolean for tests / direct use; unrelated keys return false from on_key so other handlers can claim them.

### `cli/drivers/curses/widgets/text.lua`
§M8 interpolated-text widget. `M.new({template, schema?})` parses `template` into a sequence of literal-string and `{path/segment}` placeholder segments; `{{` and `}}` escape to literal braces. `widget:paths()` enumerates the unique placeholder paths in encounter order so the driver can subscribe each one to mutation events. `widget:render(kernel)` substitutes via `ui/format.value(value, schema_td, schema)` so bounded ints inline as "cur/max" etc.; nil values become "-". `widget:paint(screen, win, kernel)` writes the substituted text top-aligned, one logical line per row, clipped on the right edge of the window — `\n` in the template becomes a real row break. A static template with no placeholders is valid (paint just writes the literal string).

### `cli/drivers/curses/widgets/inventory.lua`
§M8 scrollable list widget for Set / List / UList state paths. `M.new({path, label?, schema?, max_h?, bullet?, on_select?, fn_name?, empty_text?})` returns a view+focus-handler. Paint renders an optional header line then bulleted items (`"- item"`, bullet glyph configurable) read from `kernel:get_state(path)` (assumed sequence table); empty / nil values show `empty_text` ("(empty)" by default). The cursor (1-based highlighted row) renders with reverse attrs and the rest of the row is padded so the inversion covers the full width. Focus on_key: Up/Down move cursor, PgUp/PgDn scroll by body height, Home/End jump to extremes, Enter activates — `fn_name` (preferred) fires through `kernel:call_fn(fn_name, item)`, `on_select(self, item, idx, driver)` fires after. `max_h` caps the body height even when the window has more rows; scrolling keeps the cursor visible.

### `cli/drivers/curses/widgets/choice.lua`
§M8 radio-group widget for enum state paths. `M.new({path, values?, schema?, label?, fn_name?, on_select?, on_glyph?, off_glyph?})` returns a view+focus-handler. When `values` is omitted, the constructor walks the schema to resolve the path's enum values (inline `{tag="enum"}` or `{tag="named"}` chains, following `kind="alias"` references). Paint renders an optional header then one `"(*) value"` / `"( ) value"` row per enum value — the selected glyph (`*` by default) is driven by the kernel state, while the cursor highlight (reverse attrs) tracks a separate widget-local position that defaults to the currently-selected value. Focus on_key: Up/Down move highlight, Home/End jump to extremes, Enter / SPACE activates — `fn_name` fires through `kernel:call_fn(fn_name, value)`, then `on_select(self, value, driver)`.

### `cli/drivers/curses/widgets/narration.lua`
§M8 scrollable narration pane widget. `M.new({lines?, max_lines?, stick_to_bottom?, border?, title?})` owns its own line buffer with `append(line)` / `extend(lines)` / `clear()` / `lines()` / `size()`. `max_lines` trims the front of the buffer; default `stick_to_bottom = true` keeps the next paint anchored to the tail, but the first PgUp / Up scroll pins the view and disables stick-to-bottom until the user scrolls back to the end. Paint writes into a driver-allocated window: optional ASCII border (with title on top border) eats two rows / two cols; the body region scrolls inside; long lines are clipped on the right. Focus on_key: Up/Down by one line, PgUp/PgDn by body height, Home / End jump to extremes; returns false on unrelated keys.
...
| `tests/cli/drivers_spec.lua` | UI drivers: plain render/prompt/notify, ansi color escapes, --ui driver injection via engine.new, driver.is_driver contract, plain/ansi/curses lifecycle stubs, curses driver shape (lazy init, attach/detach, is_available), --ui whitelist incl. curses, `--ui curses --ui-backend buffer` end-to-end CLI plumbing (scripted keys, --ui-snapshot file dump, plain-driver ignores --ui-* without error) (39 tests) |
| `tests/ui/helpers/fake_curses.lua` | Shared stub of the `curses` (lcurses) module for headless tests. Exposes the slice of lcurses that `backend_curses.lua` consumes (`initscr`/`cbreak`/`echo`/`curs_set`/`endwin`/`isendwin` + `stdscr:keypad`/`getmaxyx`/`refresh`/`mvaddstr`/`getch`) and adds `_calls`/`_stdscr`/`_snapshot`/`_snapshot_row`/`_end_called` introspection helpers. Consumed by `tests/ui/curses_driver_spec.lua`; reusable by M5+ widget specs that need to exercise the ncurses path without a TTY. |
| `tests/ui/helpers/driver_session.lua` | §M5 session-DSL helper. `session.new({rows?, cols?, keys?, frame_history?})` wraps the boilerplate around `script_keys → render → prompt → snapshot` against the buffer backend so widget specs aren't 30 lines of glue. Exposes `render(scene)`, `choose(keys, choices?)`, `script_keys`, `snapshot`, `snapshot_rows`, plus the §M5 history surface (`snapshot_history`, `dirty_history`, `reset_history`) and raw `driver()`/`backend()` handles. |
| `tests/ui/kernel_spec.lua` | Presentation kernel (`ui/kernel.lua`): subscription registry (single/multi subscribers, pcall isolation, unsubscribe handle, off, wildcard, type assertions, self-unsubscribe during dispatch); state cache invariants (mirror before listener fires, accumulation, merged get_state_all); integration via lib/storybase (mutation w/ tick/seq, choice-made label, legacy "choice", scene-change reaches subscribers, cache reflects choices, initial seed, get_scene, get_schema, multi-subscriber, coalesced mutation burst) (22 tests) |
| `tests/ui/format_spec.lua` | Display formatter (`ui/format.lua`): bool (yes/no + opts.bool_labels), int (bare + cur/max + min-only), float (`%g`), value dispatch (int/bool/symbol/enum/list/set/named-alias chain), nil → "-", path schema lookup (16 tests) |
| `tests/ui/curses_driver_spec.lua` | §M3 curses driver headless tests: ncurses backend lifecycle (init/size/close idempotency, missing-lcurses path) and flush+read_key surface via a stub `curses` module; driver round-trip on the buffer backend (render → prompt → choice index, §M6 q/ESC behaviour — q+Yes confirms, q+No cancels, EOF returns nil — out-of-range key handling, end-of-game no-choices rendering, separator/choices/prompt layout, scrollback tail-clip, label width-clip); driver round-trip on the ncurses stub (paint + endwin on detach); driver protocol conformance, lazy init, unknown-backend rejection, is_available (33 tests) |
| `tests/ui/curses_screen_spec.lua` | §M3 screen model + paint diff + buffer backend unit tests: screen construction (positive dimensions, blank cells), windows + write (clipped to window + screen, negative coords no-op), fill, clear, commit (frame→prev rotation), as_lines; paint.diff (no-op when unchanged, single-span change, non-contiguous spans on the same row, attribute-only change forces dirty, invalidate forces full repaint); buffer backend (init/size, reject non-positive geometry, flush applies rects, snapshot trims trailing spaces, script_keys with strings + ints, close idempotency); golden snapshots — hello-world and ending screens compared to `tests/ui/snapshots/curses_*.txt`; dirty-rect end-to-end controlled before/after (23 tests) |
| `tests/ui/snapshots/curses_hello.txt`, `curses_ending.txt` | Golden text snapshots consumed by `curses_screen_spec.lua`. Newline-joined cell grids (trailing spaces trimmed per row, no trailing newline after the last row). Regenerate by re-running the driver + buffer backend and copying the `snapshot()` output. |
| `tests/ui/statbar_widget_spec.lua` | §M5 stat-bar widget unit tests against a fresh screen + tiny path→value kernel stub: construction (path required, label defaults to path tail or explicit), `format_head` (bounded → "Label: cur/max ", unbounded → bare, nil → "-"), paint (typical width fills proportionally, value=0 empty, value=max full, clamps under/over min/max, shrinking values erase trailing region, narrow windows render head-only-truncated, zero-height windows are a no-op), color attrs (`color_good`/`color_bad` propagate as cell `fg`) (~17 tests) |
| `tests/ui/driver_session_spec.lua` | §M5 session-DSL tests for `tests/ui/helpers/driver_session.lua` + the buffer-backend frame-history opt-in: session construction, `script_keys`/`render`/`choose` round-trip, quit on q, per-row snapshot; history is empty when off, records one entry per flush when on, `reset_history` drops both buffers, dirty rect entries are deep-copied so later flushes don't mutate earlier records (~9 tests) |
| `tests/ui/integration/statbar_reactive_spec.lua` | §M5 acceptance harness (ui_idea.md §9.1 item 3). Boots a real `ui/kernel.lua`, attaches a buffer-backed curses driver, scripts mutations through `kernel:emit`, asserts snapshot + dirty-rect log. Covers widget discovery (state `ui:` block; `ui-panel` child; same path declared in both renders once; unknown widget kinds skipped), initial paint (`Health: 7/10` from kernel cache; empty bar at 0; full bar at max), the M5 acceptance criterion (single mutation produces dirty rects all on the stat-bar row; two mutations → two flushes + decreasing `#` count; unbound path → no flush; non-mutation event → no flush), and lifecycle (detach unsubscribes; no widgets → harmless attach) (~13 tests) |
| `tests/ui/view_stack_spec.lua` | §M6 driver-local view stack unit tests: construction (empty stack), push/pop firing `on_open`/`on_close` with the driver tag, LIFO ordering, rejects pushes without `paint`, optional lifecycle hooks, `each_for_paint` walks bottom-to-top, `clear` empties top-down (10 tests) |
| `tests/ui/focus_spec.lua` | §M6 focus / key dispatch stack unit tests: construction, push/pop firing `on_focus`/`on_blur` in the right order, rejects pushes without `on_key`, dispatch routes only to top handler with no fallthrough, truthy non-boolean returns normalise to `true`, false returned when stack empty or top didn't consume, `clear` pops every handler top-down (13 tests) |
| `tests/ui/dialog_widget_spec.lua` | §M6 modal dialog widget unit tests against a fresh screen: construction (body type check, defaults, z=100 default + override); paint (centred bordered box; title + body + buttons in vertical order; body-only no buttons; multi-line body on `\n`; clipped on max_w; corner glyphs); on_key (returns false for unmatched, fires button.action + on_select for matched key, case-insensitive match, ESC/q/Q always fire on_cancel, ESC returns true even without an on_cancel, rejects non-numeric keys); zero-button degenerate case (18 tests) |
| `tests/ui/integration/quit_dialog_spec.lua` | §M6 acceptance harness. Boots a buffer-backed curses driver via the session helper, scripts q/y/n/ESC sequences and asserts the dialog appears and steers prompt() correctly: q+Y returns nil (confirmed quit); q+N+digit returns the chosen index; ESC opens dialog same as q; ESC/q inside the modal cancel it; view + focus stacks both grow/shrink in lockstep with the modal; the dialog overlays the existing game frame (narration and dialog coexist on different rows); dirty rects after q-press land in the centre rows where the dialog paints; mutation-driven recompose still paints the stat-bar widget behind the dialog; plain driver still treats q as immediate quit (10 tests) |
| `tests/ui/menu_widget_spec.lua` | §M7 menu widget unit tests against a fresh screen: construction (items required, defaults, cursor lands on first selectable, all-non-selectable → cursor 0), paint (bordered box, title on top border, divider row of glyphs, custom divider glyph, hotkey rendered as `[k] Label`, border=false suppresses box, oversized menu clips to screen), navigation (Up/Down step selectable items, skip dividers/headers, wrap on by default, wrap=false clamps, Home/End jump, `set_cursor` snaps to nearest selectable), highlight (cursor row carries `highlight_attrs`, attrs overridable), activation (Enter fires `action` + `on_activate` in order, no-op when no selectable, `activate()` returns false on header-only menu), hotkeys (jump-and-activate, non-bound keys return false, hotkey on non-selectable item ignored), cancel (ESC fires on_cancel + consumed without handler, custom cancel key works), custom bindings (vim j/k as down/up alongside arrows), non-numeric keys return false (35 tests) |
| `tests/ui/input_widget_spec.lua` | §M7 single-line input widget unit tests: construction (initial type-check, defaults, prefill, max_len trimming via `set_text`), paint (bordered box + title + prompt + buffer, cursor glyph reverse-video at insertion point, border=false suppresses, no title → top border carries no letters, oversized clips to screen), editing (printable inserts, backspace + KEY_BACKSPACE + DEL, left/right + Home/End cursor moves, forward Delete (KEY_DC), insertion at cursor (not always end), max_len cap, non-printable/non-bound keys ignored, non-numeric key returns false), submit/cancel (CR/LF/KEY_ENTER all submit with current text, ESC fires on_cancel always consumed, `fn_name` fires `driver._kernel:call_fn(name, text)` before on_submit, kernel-less driver is a no-op), customisable cancel binding, `set_text`/`submit()` helpers (32 tests) |
| `tests/ui/integration/menu_input_spec.lua` | §M7 acceptance harness. Boots a buffer-backed curses driver + real `ui/kernel.lua`, pushes menu / input widgets onto the driver's view + focus stacks, and dispatches keys through the focus stack to prove the surface: menu navigation under focus (Up/Down/Enter, hotkey jump-and-activate against arbitrary highlight, menu paints over base game frame); input end-to-end (typing `"Alice"` + Enter fires `kernel:call_fn("set-name", "Alice")` via `set_call_fn` capture; backspace edits before submit; ESC cancels without invoking call_fn; on_submit can pop both stacks; widget paints over narration); kernel `call_fn` host plumbing (no handler → nil; `set_call_fn` replaces previous) (11 tests) |
| `tests/ui/stat_widget_spec.lua` | §M8 generic-stat widget unit tests against a fresh screen + fake kernel: construction (path required, label defaults to path tail, explicit label override), format (bounded int → "cur/max", unbounded → bare, bool → yes/no, enum → symbol, nil → "-", named alias resolved via schema), paint (typical render, nil → "-", truncation, shrinking values erase trailing region, zero-height window no-op) (18 tests) |
| `tests/ui/toggle_widget_spec.lua` | §M8 Bool toggle widget unit tests: construction (path required, label defaults / overrides, default + custom glyphs first-char only), format (`[x] Label` / `[ ] Label`, nil → off), paint (truthy / falsy / nil render, long label truncated), activate (fn_name fires `kernel:call_fn(name, new)`, on_toggle receives new value, both fire when both set), focus on_key (SPACE / CR / LF / KEY_ENTER all activate; unrelated keys return false without firing callbacks) (17 tests) |
| `tests/ui/text_widget_spec.lua` | §M8 interpolated-text widget unit tests: parsing (template required, static template has no paths, multi-placeholder paths deduped in encounter order, unterminated `{` errors, empty `{}` errors, `{{` / `}}` escape to literal braces), render (single + multi placeholder substitution, nil → "-", schema type-descriptor lookup for "cur/max", bool formatter integration), paint (static line, substituted line, multi-line `\n` honoured, right-edge truncation, zero-width / zero-height no-op) (16 tests) |
| `tests/ui/inventory_widget_spec.lua` | §M8 inventory widget unit tests: construction (path required, default bullet `-` / empty_text `(empty)`, single-char bullet override), paint empty (missing value, empty table) renders empty_text, paint non-empty (bullets + items, cursor row gets reverse attrs, scrolls when cursor moves past visible body), activate (on_select / fn_name fire with item + idx, returns false for empty list), focus on_key (Up/Down moves cursor, Home/End jump, Enter activates, unrelated keys return false) (17 tests) |
| `tests/ui/choice_widget_spec.lua` | §M8 radio-group widget unit tests: construction (path required, values required when no schema, explicit values accepted, inline-enum schema resolution, named-alias chain resolution), paint (header label row, selected glyph mirrors kernel state, all-off when value is nil, cursor highlight independent of selection), activate (fn_name fires `kernel:call_fn` with chosen value, on_select fires too), focus on_key (Up/Down move highlight + clamp at extremes, Home/End jump, CR / LF / KEY_ENTER / SPACE all activate, unrelated keys return false) (15 tests) |
| `tests/ui/narration_pane_widget_spec.lua` | §M8 scrollable narration pane unit tests: buffer (empty start, opts.lines seed, append/extend/clear, max_lines trims front), paint (tail when shorter than body, stick-to-bottom when buffer exceeds body, optional border + title with corner glyphs, long-line clipping to inner_w), scroll (scroll(-1) pins / disables stick_to_bottom, scrolling back to bottom re-enables, scroll_to(false/true) jumps), focus on_key (Up/Down by one, PgUp/PgDn by body_h, Home / End jump, unrelated keys return false) (14 tests) |
| `tests/ui/integration/m8_widgets_spec.lua` | §M8 acceptance harness. Boots a buffer-backed curses driver + real kernel and verifies schema-driven discovery + reactive repaint for each new widget kind: stat (`stat` ui block renders `"Label: value"`; mutation repaints just the widget row); toggle (`[x]`/`[ ]` mirror Bool mutations); text (panel `text` child substitutes `{path}` and subscribes to every referenced path); inventory (list state renders bullets + label; mutation on the list repaints); choice (enum schema renders `(*)`/`( )` rows with selection mirroring kernel state); layout (multiple widgets stack top-to-bottom; inventory + stat coexist at stable y coords) (10 tests) |
...
| `demo35_healers_ward_ui.sb` | §M9 showcase: author-declared UI bindings end-to-end. Forks demo13 (the healer's ward) and adds `ui:` blocks on supply / day / focus / auto-rest / journal state plus a `ui-panel ward-hud` that composes them with a `text` widget. Exercises every widget kind in ui_idea.md §5.3: `stat-bar` (`world/herbs`, `world/bandages` — bounded `Int` with bar + cur/max), `stat` (`world/day`), `choice` (`world/triage-focus` enum, radio group; curses-only), `toggle` (`world/auto-rest` Bool wired to `fn: set-auto-rest` so space/Enter round-trips through the engine), `inventory` (`world/journal` as `UList(String)` with `max-rows: 5`), and `text` (interpolated `"Tending: {world/selected-patient} (day {world/day})"`). `engine-config: ui-runtime: true` keeps the bindings live under `--production`. Under `--ui curses` the full HUD strip stacks at the top and each `mutation` repaints just the affected widget row; under `--ui plain` stat / stat-bar / toggle / inventory / text flatten to one header line each (choice has no flat rendering). Pairs with `docs/howto/ui_bindings.md` (author surface) and `docs/reference/driver_protocol.md` (driver-implementor surface). |
