...
Curses UI driver (`--ui curses`) — pull-mode driver built on the §M3 screen model + dual backends. Composes one frame per `render`/`prompt`: narration tail / separator / numbered choices / prompt status line, painted into a retained 2D `screen.lua` grid; `paint.lua` diff-flushes only changed cells to the chosen backend. `opts.backend` selects "ncurses" (default) or "buffer" (headless tests, no-tty fallback). `M.new` is lazy: `backend:init` is deferred to the first `render`/`prompt` so `is_driver()` checks work without a TTY. `opts.curses` injects a stub curses module for the ncurses backend; `opts.keys` pre-loads the buffer backend's key queue. `attach(kernel)` stores the kernel handle; the reactive-widget path lands at §M5. Registered in the `cli.ui` enum (`runtime/engine.lua`).
