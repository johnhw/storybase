...
Curses UI driver (`--ui curses`) — pull-mode hello-world per ui_idea.md §M2. Delegates all rendering and input to `backend_curses.lua`; itself contains zero curses calls. `M.new(opts)` is lazy: `initscr()` is deferred to the first `render`/`prompt` so `is_driver()` checks and tests work without a TTY. `opts.curses` injects a stub curses module for headless tests. Registered in the `cli.ui` enum (`runtime/engine.lua`). The kernel-driven `attach`/`tick`/`detach` path remains a forward-compatible stub; §M3 will wire it once the screen model lands.

### `cli/drivers/curses/backend_curses.lua`
ncurses backend for the curses driver — **the only file in the project that `require("curses")`**. Provides `M.is_available()` (probes packaging), `M.new(opts)` (lazy curses resolution), and a backend with `init`/`close`/`append_narration`/`paint(choices, choices_active)`/`read_choice(num_choices)`. Lifecycle is idempotent. Paint layout: narration scrollback (tail-clipped to terminal rows), separator dashes, numbered choices, prompt status line at the bottom. Input loop accepts `1`–`9` for choice indices and `q`/`Q`/`ESC` for quit. Headless tests in `tests/ui/curses_driver_spec.lua` inject a stub curses via `opts.curses`.
...
| `tests/ui/curses_driver_spec.lua` | §M2 curses driver headless tests via a stub curses module: backend lifecycle (init/close idempotency, missing-lcurses path), narration accumulation (say/narration/multi-line/skip-empty), paint layout (scrollback tail, separator, choices, prompt active vs inactive, width clipping), read_choice dispatch (digits, q/Q/ESC, out-of-range filtering, EOF), driver render+prompt round-trip, detach cleanup (28 tests) |
