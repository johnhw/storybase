...
Plain-text UI driver (default). Renders narration as `"Speaker: text"` or bare text; numbered choice prompt; reads from stdin. §M5: `attach(kernel)` discovers author-declared `stat-bar` bindings (state `ui:` blocks + `ui-panel` children) and prefixes every `render` with one `"Label: cur/max"` line per binding, re-read from the kernel state cache (no mutation subscription — the driver is synchronous-pull). `detach` drops the bindings.
...
Curses UI driver (`--ui curses`) — pull-mode for narration + choices, reactive for §M5 widgets, with a §M6 view-stack for overlays. Composes one frame per `render`/`prompt`: optional stat-bar strip (top) / narration tail / separator / numbered choices / prompt status line, then each active view (e.g. modal dialog) painted on top via `_view_stack:each_for_paint`, into a retained 2D `screen.lua` grid; `paint.lua` diff-flushes only changed cells. `opts.backend` selects "ncurses" (default) or "buffer" (headless tests, no-tty fallback). `opts.frame_history` opts the buffer backend into per-flush snapshot + dirty-rect recording. `M.new` is lazy: `backend:init` is deferred to the first `render`/`prompt` so `is_driver()` checks work without a TTY. `opts.curses` injects a stub curses module for the ncurses backend; `opts.keys` pre-loads the buffer backend's key queue. `attach(kernel)` scans the schema (state `ui:` blocks) and `ui_panels` (children with state-path refs), instantiates a widget per binding (today: `stat-bar` via `widgets/statbar.lua`), and subscribes to `mutation` so a state change recomposes the frame — windows for narration/choices/prompt re-paint identically and diff clean, so the dirty-rect log covers only the widget window. §M6 input dispatch in `_read_choice`: keys route to the top focus handler first (modal dialogs intercept); if not consumed, q/Q/ESC open a quit-confirm dialog via `_open_quit_dialog` (Yes sets `_quit_pending` and returns nil from prompt; No/ESC just pop the modal); digits 1..#choices resolve the prompt. `detach` clears view + focus stacks, drops subscriptions, and tears down the backend. Registered in the `cli.ui` enum (`runtime/engine.lua`).
