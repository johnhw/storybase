...
Curses UI driver (`--ui curses`) — pull-mode for narration + choices, reactive for §M5 widgets. Composes one frame per `render`/`prompt`: optional stat-bar strip (top) / narration tail / separator / numbered choices / prompt status line, painted into a retained 2D `screen.lua` grid; `paint.lua` diff-flushes only changed cells to the chosen backend. `opts.backend` selects "ncurses" (default) or "buffer" (headless tests, no-tty fallback). `opts.frame_history` opts the buffer backend into per-flush snapshot + dirty-rect recording. `M.new` is lazy: `backend:init` is deferred to the first `render`/`prompt` so `is_driver()` checks work without a TTY. `opts.curses` injects a stub curses module for the ncurses backend; `opts.keys` pre-loads the buffer backend's key queue. `attach(kernel)` scans the schema (state `ui:` blocks) and `ui_panels` (children with state-path refs), instantiates a widget per binding (today: `stat-bar` via `widgets/statbar.lua`), and subscribes to `mutation` so a state change recomposes the frame — windows for narration/choices/prompt re-paint identically and diff clean, so the dirty-rect log covers only the widget window. `detach` drops subscriptions and tears down the backend. Registered in the `cli.ui` enum (`runtime/engine.lua`).
...
| `tests/ui/curses_driver_spec.lua` | §M3 curses driver headless tests: ncurses backend lifecycle (init/size/close idempotency, missing-lcurses path) and flush+read_key surface via a stub `curses` module; driver round-trip on the buffer backend (render → prompt → choice index, q/ESC/out-of-range key handling, end-of-game no-choices rendering, separator/choices/prompt layout, scrollback tail-clip, label width-clip); driver round-trip on the ncurses stub (paint + endwin on detach); driver protocol conformance, lazy init, unknown-backend rejection, is_available (31 tests) |
